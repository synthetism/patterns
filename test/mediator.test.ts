...
describe("Mediator Pattern", () => {
	...
	it("should handle void commands (no return value)", async () => {
		...
		expect(logHandler.getLoggerMock().info).toHaveBeenCalledWith(
			"Test message",
		);
	});
	...
	it("should throw error when no handler is registered", async () => {
		await expect(mediator.send({ type: "UNKNOWN_COMMAND" })).rejects.toThrow(
			"No handler registered for request type: UNKNOWN_COMMAND",
		);
	});
