		...
		describe("defined", () => {
			...
			it("should fail for null values", () => {
				...
				expect(result.errorMessage).toBe(
					"testValue cannot be null or undefined",
				);
			});
	...
	describe("String Guards", () => {
		...
		describe("maxLength", () => {
			...
			it("should fail for strings that are too long", () => {
				const result = Guard.String.maxLength(
					"toolongusername",
					10,
					"username",
				);
		...
		describe("length", () => {
			...
			it("should fail for strings that are too long", () => {
				const result = Guard.String.length(
					"verylongusername",
					3,
					10,
					"username",
	...
	describe("Array Guards", () => {
		...
		describe("maxLength", () => {
			...
			it("should fail for arrays with too many items", () => {
				...
				expect(result.isFailure).toBe(true);
				expect(result.errorMessage).toBe(
					"items must not contain more than 3 items",
				);
			});
		});
		...
		describe("includes", () => {
			...
			it("should fail when array does not include required item", () => {
				...
				expect(result.isFailure).toBe(true);
				expect(result.errorMessage).toBe(
					"items does not contain the required item",
				);
			});
		});
	...
	describe("Object Guards", () => {
		describe("hasProperty", () => {
			...
			it("should fail for objects missing specified property", () => {
				...
				expect(result.isFailure).toBe(true);
				expect(result.errorMessage).toBe(
					"user does not have the required property 'name'",
				);
			});
		});
