...
describe("Result Pattern", () => {
	...
	describe("Method chaining", () => {
		it("should call onSuccess for success result", () => {
			...
			const result = Result.success(value).onSuccess((v) => {
				wasCalled = true;
				expect(v).toBe(value);
			});
		...
		it("should not call onSuccess for failure result", () => {
			...
			const result = Result.fail<string>("Error").onSuccess(() => {
				wasCalled = true;
			});
		...
		it("should call onFailure for failure result", () => {
			...
			const result = Result.fail<number>(errorMessage, cause).onFailure(
				(msg, err) => {
					wasCalled = true;
		...
		it("should not call onFailure for success result", () => {
			...
			const result = Result.success(42).onFailure(() => {
				wasCalled = true;
			});
	...
	describe("ensure method", () => {
		it("should return the result if condition is met", () => {
			const result = Result.success(42).ensure(
				(v) => v > 0,
		...
		it("should return failure if condition is not met", () => {
			const result = Result.success(0).ensure(
				(v) => v > 0,
				"Value must be positive",
			);
		...
		it("should not check condition for failure results", () => {
			...
			const result = Result.fail<number>("Already failed").ensure(() => {
