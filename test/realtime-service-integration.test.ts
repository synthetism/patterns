...
class MockRealtimeChannel {
  ...
  simulatePostgresEvent(eventType: 'INSERT' | 'UPDATE' | 'DELETE', payload: any) {
...
describe('Supabase RealtimeService Integration', () => {
  ...
  it('should update cache when insert events occur', () => {
    ...
    expect(cache.get('user:1')).toEqual({ id: 1, name: 'John', email: 'john@example.com' });
  });
  ...
  it('should update cache when update events occur', () => {
    ...
    const cache = cacheService.getCache();
    expect(cache.has('user:1')).toBe(true);
    expect(cache.get('user:1')).toEqual({ id: 1, name: 'John Updated', email: 'john@example.com' });
  });
  ...
  it('should handle multiple events for multiple entities', () => {
    ...
    expect(postObserver.update).toHaveBeenCalledWith(expect.objectContaining({
      type: 'post.insert',
      entityName: 'post',
      eventType: 'insert',
