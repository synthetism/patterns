...
export class WebSocketRealtimeServer implements RealtimeServer {
  ...
  constructor(private options: RealtimeServerOptions<WebSocketServerOptions> = {}) {
    this.stats = {
      connectedClients: 0,
      totalTopics: 0,
      messagesSent: 0,
  ...
  async broadcast(topic: Topic, event: RealtimeEvent): Promise<void> {
    ...
    console.log(`Broadcasted to ${topic}: ${sentCount} clients received event`);
  }
  ...
  private handleNewConnection(socket: WebSocket, request: IncomingMessage): void {
    ...
    console.log(`Client ${clientId} connected and subscribed to: ${topic}`);
