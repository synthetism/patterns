...
export class NatsRealtimeServer implements RealtimeServer {
  ...
  constructor(private options: RealtimeServerOptions<NatsServerOptions> = {}) {
    this.stats = {
      connectedClients: 0,
      totalTopics: 0,
      messagesSent: 0,
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      console.log(`NATS RealtimeServer connected to ${natsUrl}`);
  ...
  async stop(): Promise<void> {
    ...
    console.log("NATS RealtimeServer stopped");
  }
  ...
  async broadcast(topic: Topic, event: RealtimeEvent): Promise<void> {
    ...
    try {
      ...
      // Log broadcast
      console.log(`Broadcasted to ${topic}: Event ${event.type}`);
    } catch (error) {
  ...
  async sendToClient(clientId: string, event: RealtimeEvent): Promise<void> {
    ...
    try {
      ...
      // Log
      console.log(`Sent direct message to client ${clientId}: ${event.type}`);
    } catch (error) {
  ...
  private async setupControlSubscriptions(): Promise<void> {
    ...
    try {
      ...
      (async () => {
        for await (const msg of disconnectSub) {
          try {
            ...
            console.error("Error processing client disconnection message:", error);
          }
        }
      })().catch(err => console.error("Error in control.disconnect subscription:", err));
      ...
      })().catch(err => console.error("Error in control.subscribe subscription:", err));

      // Subscribe to unsubscription requests
      const unsubscribeSub = await this.natsConnection.subscribe("control.unsubscribe");
      ...
      })().catch(err => console.error("Error in control.unsubscribe subscription:", err));

      console.log("Control subscriptions set up");
    } catch (error) {
      console.error("Failed to set up control subscriptions:", error);
      throw error;
    }
  }
  ...
  private handleClientConnection(connectionData: ControlConnectMessage, replySubject?: string): void {
    ...
    console.log(`Client ${clientId} connecting to topic: ${chalk.bold(topic)}`);
    ...
    console.log(`Client ${clientId} connected and subscribed to: ${chalk.bold(topic)}`);
  ...
  private async createTopicSubscription(topic: Topic): Promise<void> {
    ...
    try {
      ...
      })().catch(err => console.error(`Error in topic.${topic} subscription:`, err));
      
      console.log(`Created subscription for topic: ${topic}`);
    } catch (error) {
      console.error(`Failed to create subscription for topic ${topic}:`, error);
    }
  }
  ...
  private removeTopicSubscription(topic: Topic): void {
    ...
    if (subscription) {
      ...
      console.log(`Removed subscription for topic: ${topic}`);
    }
  }
   ...
   */
  private async forwardMessageToSubscribers(topic: Topic, event: RealtimeEvent): Promise<void> {
    if (!this.natsConnection) return;
    ...
    console.log(`Forwarded message to ${sendPromises.length} clients on topic ${topic}`);
  }
